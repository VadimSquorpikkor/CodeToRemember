...
/////////////ЗАРАБОТАЛ ПОСЛЕ ЗАМЕНЫ INT НА DOUBLE(НИЧЕМ БОЛЬШЕ НЕ ОТЛИЧАЕТСЯ ОТ ВЕРСИИ 1)
...
public class GreedyKnapsack_v2 {
    ...
    double calc() throws FileNotFoundException {
        ...
        double W = input.nextDouble();      //какой вес у рюкзака
        Item[] items = new Item[n];   //получим список предметов
        for (int i = 0; i < n; i++) { //создавая каждый конструктором
            items[i] = new Item(input.nextDouble(), input.nextDouble());
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        double costFinal=new GreedyKnapsack_v2().calc();
